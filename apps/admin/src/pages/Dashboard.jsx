    ...
    <Shell>
      ...
      <div className="pp-between pp-wrap ck-ai-end">
        <div>
          ...
          <p className="ck-sub ck-mb-0">{eventDate(event.starts_at)} · {event.city}/{event.state}</p>
        </div>
      ...
      <nav aria-label="Estações do evento" className="ck-tabs ck-mt-4">
        {STATIONS.map((s) => (
      ...
      {publicarBloqueado && (
        <div className="ck-card ck-mt-5 ck-w-mid ck-aviso">
      ...
      <div className="ck-kpis ck-mt-5">
        <div className="ck-kpi ck-k--pulse">
          ...
          <div className="val ck-c-pulse">{brl(m.total_revenue_cents)}</div>
        ...
        <div className="ck-kpi ck-k--pink">
      ...
      <div className="ck-duo ck-mt-4">
        <section className="ck-panel" aria-label="Vendas por lote">
          <div className="pp-between">
            ...
            <span className="pp-price ck-t-section">{brl(m.ticket_revenue_cents)}</span>
          </div>

          <div className="pp-stack pp-stack-3 ck-mt-5">
                ...
                <div key={t.id}>
                  <div className="pp-between ck-mb-2">
                    <span className="pp-row ck-gap-2">
                      <span aria-hidden="true" className="ck-dot" style={{ background: tone }} />
                      <span className="ck-w-semi ck-t-support">{t.name}</span>
                      <span className={`ck-badge ${t.status === 'on_sale' ? 'ck-badge--published' : ''} ck-t-label`}>{t.status}</span>
                    </span>
                  ...
                  <div className="ck-bar" style={{ '--k': tone }}>
                    ...
                    <span className="pp-mono pp-num pp-muted-2 ck-right ck-t-support ck-col-pct">{Math.round(pct)}%</span>
                  </div>
                </div>
              );
            })}
        ...
        <div className="pp-stack pp-stack-4">
          ...
          {occ && (
            <section className="ck-panel" aria-label="Ocupação ao vivo">
              ...
              <div className="pp-between ck-mt-4 ck-ai-base">
                <span className="ck-t-money">{occ.inside}</span>
                <span className="pp-mono pp-muted ck-t-support">de {occ.tickets_sold} ingressos</span>
              </div>
              <div className="ck-bar ck-mt-2 ck-k--pulse">
                <div className="track">
                  <div className="fill" style={{ width: `${occ.tickets_sold > 0 ? Math.min(100, (occ.inside / occ.tickets_sold) * 100) : 0}%` }} />
                </div>
              </div>
              <div className="pp-between ck-mt-3">
                <span className="pp-muted ck-t-support">{occ.total_entries} entradas no total</span>
                <span className="pp-muted ck-t-support">{occ.left} saíram</span>
              </div>
            </section>
          )}

          <section className="ck-panel" aria-label="Pedidos recentes pagos">
            <div className="ck-eyebrow ck-mb-3">Pedidos recentes · pagos</div>
            {dash.recent_orders.length === 0 && (
              <p className="pp-muted ck-m-0">
              ...
              {dash.recent_orders.map((o) => (
                <div key={o.id} className="ck-feed__row">
                  ...
                  <div className="pp-grow">
                    <div className="pp-mono ck-t-support">{o.id.slice(0, 8)}</div>
                    <div className="pp-muted-2 ck-t-support">{dateTime(o.created_at)}</div>
                  </div>
                  <span className="pp-price ck-t-support">{brl(o.total_cents)}</span>
                </div>
              ))}
            </div>
          </section>
