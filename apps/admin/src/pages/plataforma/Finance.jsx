    ...
    <AdmShell where="Receita da plataforma · repasses">
      {error ? <ErrorBox>{error}</ErrorBox> : !fin ? <Loading /> : (
        <div className="pp-stack pp-stack-5 pp-reveal">
          <div>
            <div className="adm-eyebrow" style={{ color: 'var(--pp-pulse)' }}>Financeiro</div>
            <div className="adm-h1">Receita e <span className="accent" style={{ color: 'var(--pp-pulse)' }}>repasses</span></div>
          </div>
          ...
          <div className="adm-panel">
            <div style={{ fontFamily: 'var(--pp-font-display)', fontWeight: 600, fontSize: 'var(--pp-fs-18)' }}>Repasse por organização</div>
            <p className="pp-muted" style={{ fontSize: 'var(--pp-fs-12)', margin: '2px 0 var(--pp-s-4)' }}>GMV menos a taxa da plataforma = líquido a repassar.</p>
            <table className="ck-table">
              ...
              <tbody>
                {fin.payouts.length === 0 && <tr><td colSpan={4} className="pp-muted" style={{ padding: 'var(--pp-s-5)' }}>Sem vendas ainda.</td></tr>}
                {fin.payouts.map((p) => (
                  <tr key={p.id}>
                    <td><b>{p.name}</b></td>
                    <td className="num pp-mono">{brl(p.gmv_cents)}</td>
                    <td className="num pp-mono" style={{ color: 'var(--pp-pink)' }}>{brl(p.fee_cents)}</td>
